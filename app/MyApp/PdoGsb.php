...
public function getInfosGestionnaire($login, $mdp){
	$req = "select gestionnaire.nom as nom, gestionnaire.prenom as prenom from gestionnaire 
 ...
 */
public function ajoutNouveauVisiteur($idVisiteur, $nom, $prenom, $login, $mdp, $adresse, $cp, $ville, $dateEmbauche){
        
	$req = "insert into visiteur(id, nom, prenom,login, mdp, adresse, cp, ville, dateEmbauche) 
	values('$idVisiteur', '$nom', '$prenom','$login', '$mdp', '$adresse', '$cp', '$ville', 
	'$dateEmbauche')";
...
/**
* Ajouter un update visiteur
*/
public function modifierVisiteur($idVisiteur, $adresse, $cp, $ville, $dateEmbauche){

	$req = "update visiteur set adresse = '$adresse' ,cp = '$cp', ville = '$ville',dateEmbauche ='$dateEmbauche'
	where id = '$idVisiteur'";
	$res = $this->monPdo->exec($req);
...
// Génération d'une chaine aléatoire mot de passe
function generer_mdp($nb_car)
{   
...
// Génération d'un identifiant visiteur une lettre et 3 chiffres
function generer_idVisiteur()
{   
	$chaine='azertyuiopqsdfghjklmwxcvbn'; 
	$nb_lettres = strlen($chaine) - 1;

	$pos= mt_rand(0, $nb_lettres);
	$idVisiteur = $chaine[$pos];
	$idVisiteur .= random_int(0,999);
	return $idVisiteur;
}
// Génération d'une chaine aléatoire mot de passe
...
public function getInfoUnVisiteur($visiteur){
